...
- (void)viewDidLoad {
    ...
    self.navigationView.rightTitle = @"提交";
    [self.navigationView.rightButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
...
- (UIView *)contactView{
    if (!_contactView) {
        ...
        _contactView.frame = CGRectMake(0, self.bottomView.frame.origin.y + self.bottomView.height + 22, SCREEN_WIDTH, self.view.height - self.bottomView.frame.origin.y - self.bottomView.height - 22);
...
{
    if (!_contentView) {
        _contentView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, self.navigationView.height, SCREEN_WIDTH, SCREEN_HEIGHT - self.navigationView.height)];
        _contentView.contentSize = CGSizeMake(SCREEN_WIDTH, _contentView.height + 2);
