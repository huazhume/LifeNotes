...
- (void)getCodeRequest {
    ...
    [self countDownBegin];
    zl_weakify(self);
    [AFNManager POST:@"https://api.wdgcoin.com/Home/Reg/sendMobile" params:@{@"mobile" : account} complete:^(APPResult *result) {
        zl_strongify(self);
    }];
...
- (void)validateRequest {
    ...
    zl_weakify(self);
    [AFNManager POST:@"" params:param complete:^(APPResult *result) {
        zl_strongify(self);
...
- (void)codeValueChange:(UITextField *)code {
    if (code.text.length > 4) {
        code.text = [code.text substringToIndex:4];
    }
    [self buttonCanTap:code.text.length == 4];
}
